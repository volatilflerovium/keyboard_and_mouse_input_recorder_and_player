...
BasicPopup::BasicPopup(wxWindow* parent, const char* title)
...
void PopupWrapper::Popup()
{
	init();

	wxRect rect=wxDisplay().GetGeometry();
	wxSize sz=m_popupPtr->GetSize();
	wxPoint result(
		(rect.GetWidth()-sz.x)/2,
		(rect.GetHeight()-sz.y)/2
	);

	m_popupPtr->SetPosition(m_popupPtr->FromDIP(result));
	m_popupPtr->Popup();
}
...
WX_Popup::WX_Popup(wxWindow *parent, const char* title)
